...
**	识别当前使用的Flash页与段
***********************************/
bool Get_Page_Sement(void)
{
	bool see_ok = false;
	uint16_t count;

	for(pitch_segment = 0; pitch_segment < Semgment_Quantity; pitch_segment++)
	{
		see_ok = true;
		Read_Flash(pitch_segment, buff_data, Buff_Size);
		for(count = 0; count < Buff_Size; count++)
		{
			if(buff_data[count] != 0xFFFFFFFF)
			{
				see_ok = false;
				break;
			}
	...
	if(see_ok)
	{
		...
		{
			pitch_segment = Semgment_Quantity - 1;//正常情况不会出现首段未被写过(第一次刷入程序时可能)
			return false;
		}
	}
	else
	{
		pitch_segment = Semgment_Quantity - 1;
		return true;
	}
}
...
bool Store_Flash_To_RAM(bool updata)
{
	...
	if(updata)
	{
		...
		control.drive_module= *(uint32_t*)&	buff_data[count++];
...
bool Store_RAM_To_Flash(bool updata)
{
	...
	//使用新Flash段
	pitch_segment++;
	if(!(pitch_segment < Semgment_Quantity))
	{
		pitch_segment = 0;
		Empty_Flash();
	}
	...
	buff_data[count++]	= *(uint32_t*)&		control.drive_module;
