...
char uart_tx_buff[129];
char uart_rx_buff[129];
...
{
	sprintf(uart_tx_buff, "0x%4x ;", data);
	uart_tx_size = strlen((char*)uart_tx_buff);
	while(tx_huart.gState != HAL_UART_STATE_READY);//等待DMA
	//HAL_UART_Transmit(&tx_huart, (uint8_t*)uart_tx_buff, uart_tx_size, 1000000);
	HAL_UART_Transmit_DMA(&tx_huart, (uint8_t*)uart_tx_buff, uart_tx_size);
}
...
{
	sprintf(uart_tx_buff, "%3.6f ;", data);
	uart_tx_size = strlen((char*)uart_tx_buff);
	while(tx_huart.gState != HAL_UART_STATE_READY);//等待DMA
	//HAL_UART_Transmit(&tx_huart, (uint8_t*)uart_tx_buff, uart_tx_size, 1000000);
	HAL_UART_Transmit_DMA(&tx_huart, (uint8_t*)uart_tx_buff, uart_tx_size);
...
{
	uart_tx_size = strlen((char*)spr);
	strcpy(uart_tx_buff, spr);
	while(tx_huart.gState != HAL_UART_STATE_READY);//等待DMA
	//HAL_UART_Transmit(&tx_huart, (uint8_t*)uart_tx_buff, uart_tx_size, 1000000);
	HAL_UART_Transmit_DMA(&tx_huart, (uint8_t*)uart_tx_buff, uart_tx_size);
}

void SerialPort_Send_TX(void)
{
	uart_tx_size = strlen((char*)uart_tx_buff);
	while(tx_huart.gState != HAL_UART_STATE_READY);//等待DMA
	//HAL_UART_Transmit(&tx_huart, (uint8_t*)uart_tx_buff, uart_tx_size, 1000000);
...
bool SerialPort_Refer_TX_Leisure(void)
{
	if(tx_huart.gState == HAL_UART_STATE_READY)
		return true;
	else
		return false;
}
